...
def plot_solutions(fig,noise_levels:list,training_sizes:list,classification_errors:dict,rkhs_norms:dict):
    ...
    for i, noise in enumerate(noise_levels):
        ...
        ax.plot(training_sizes, classification_errors[noise]["NN"], label="Shallow NN", linestyle="-.", marker="x", color="orange")
    ...
    for i,noise in enumerate(noise_levels):
        ...
        ax_rkhs.plot(training_sizes, rkhs_norms[noise]["NN"], label=f"Shallow NN (Noise={int(noise * 100)}%)", color=colors[i], linestyle="-.", marker="x")
